...
def optimise_model_fit(raw_spec_filename, raw_spectra, region, wavelength_region, guess,star_name,linelist,vsini,Fe,CN,CC,stronglines):
    ...
    generate_parameter_string(raw_spec_filename, in_filename, out_filename, wavelength_region, par,star_name,linelist,vsini,Fe,CN,CC,stronglines)
...
def initial_guess(vpass,star_name,MgH):
    ...
    s = weighted_avg_file[weighted_avg_file['star'] == star_name]['s'].values[0]
    mg = MgH
    i_24 = weighted_avg_file[weighted_avg_file['star'] == star_name]['i_24'].values[0]
    i_25 = weighted_avg_file[weighted_avg_file['star'] == star_name]['i_25'].values[0]
    i_26 = weighted_avg_file[weighted_avg_file['star'] == star_name]['i_26'].values[0]
    rv = 0
...
for star_name in star_list:
    ...
    for region in regions:
        ...
        # coarse search
        model_finder(star_name,linelist,region, stronglines,vsini, MgH,Fe,CN,CC,vpass)
